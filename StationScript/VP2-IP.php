...
{	// la connection est faite reste a verifier si la VP2 repond
	...
	{//on test si la vp2 est reveillée (conf chapitre IV. p.5)
		...
		fwrite ($fp,"LAMPS 1\n");
		Waiting (4, 'Verification de la disponibilitée.');
		//on allume l'ecran pour suivre la connexion
		$LED=fread($fp,6);
		if (strlen($LED) && $LED==$symb['LFCR'].'OK'.$symb['LFCR'])
		{	//si on a bien confirmé, la connexion avec la VP2 est disponible
			$av = true;
			echo date('Y/m/d H:i:s u')."\t".'VP2 Disponible, Activation du retroeclairage.'."\n";
...
//	echo date('Y/m/d H:i:s u')."\tReponce ".strlen($HILOWS)." Octes :\n0x".dechex($crc0).' = 0x'.dechex($crc1)."\n";
		...
		{
			echo date('Y/m/d H:i:s u')."\t".'NULL  '.'$r = 0x'.dechex($r)."\n";
			fread($fp, 999);
...
{// le CRC16 d'ųne chaine + son CRC est toujours = a 0x0000
	...
	echo date('Y/m/d H:i:s u')."\t".'Validation des donnees recues.'."\n";
